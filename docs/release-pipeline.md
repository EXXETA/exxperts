...
- generates three CycloneDX SBOMs from the lockfiles (`sbom-app.cdx.json`, `sbom-runtime.cdx.json`, `sbom-desktop.cdx.json`) and attaches them to the release right after publish. The weekly OSV scan and on-demand SBOM artifacts live in `sbom.yml`; release attachment lives here because a release created by the workflow token does not trigger other workflows.
