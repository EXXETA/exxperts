...
async function main(argv = process.argv.slice(2), command = path.basename(process.argv[1] || "exxperts-cli")) {
  ...
  // home, or the moved location / EXXPERTS_DATA_DIR) first, so the profile
  ...
  try {
    ...
    dataFolder = stateProfiles.adoptStateHome(os.homedir()).home;
  } catch (err) {
    console.error(`\n  ${err.message}\n`);
    process.exit(1);
  }
  ...
  // launcher and desktop shell use. Pointing this process's own state home at
  // the profile before ANYTHING resolves a state path (stateHome() reads the
  // env per call) points every read (rooms, locks, picker, provider setup)
  // and every spawned runtime child at the profile's tree, while HOME stays
  // the person's own. This must precede the setup/package branch below, or
  // `exxperts setup` would write provider auth into the standard ~/.exxperts
  // while a profile is loaded.
  const activeProfile = stateProfiles.readActiveProfile(dataFolder);
  if (activeProfile !== null) {
    Object.assign(process.env, stateProfiles.serverEnvForProfile(dataFolder, activeProfile));
