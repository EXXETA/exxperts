...
export interface OpenAICompletionsCompat {
	...
	/** Whether the provider supports OpenAI-style long prompt cache retention (`prompt_cache_retention: "24h"`). Default: follows `supportsLongCacheRetention` for backward compatibility. */
	supportsOpenAIPromptCacheRetention?: boolean;
	/** Whether the provider supports Anthropic-style long cache retention (`cache_control.ttl: "1h"`) when `cacheControlFormat` is `"anthropic"`. Default: follows `supportsLongCacheRetention` for backward compatibility. */
	supportsAnthropicCacheControlTtl?: boolean;
	/** Legacy fallback for long prompt cache retention. For OpenAI Completions-compatible APIs, prefer `supportsOpenAIPromptCacheRetention` and `supportsAnthropicCacheControlTtl`. Default: true. */
