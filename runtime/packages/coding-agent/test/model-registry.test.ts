...
import type { AnthropicMessagesCompat, Api, Context, Model, OpenAICompletionsCompat } from "@exxeta/exxperts-ai";
import { getApiProvider } from "@exxeta/exxperts-ai";
import { getOAuthProvider } from "@exxeta/exxperts-ai/oauth";
...
describe("ModelRegistry", () => {
	...
	describe("custom models merge behavior", () => {
		...
		test("non-built-in provider custom models can load without apiKey in models.json", () => {
			writeRawModelsJson({
				"custom-gateway": {
					name: "Custom Gateway",
			...
			const registry = ModelRegistry.create(authStorage, modelsJsonPath);
			expect(registry.getError()).toBeUndefined();
			expect(registry.find("custom-gateway", "gpt-5.5")).toBeDefined();
			expect(registry.find("custom-gateway", "claude-opus-4.6")).toBeDefined();
			expect(registry.getAvailable().some((m) => m.provider === "custom-gateway")).toBe(false);
			expect(registry.getProviderAuthStatus("custom-gateway")).toEqual({ configured: false });
		});

		test("non-built-in provider custom models use stored auth from auth.json without apiKey in models.json", async () => {
			authStorage.set("custom-gateway", { type: "api_key", key: "stored-gateway-key" });
			writeRawModelsJson({
				"custom-gateway": {
					name: "Custom Gateway",
					baseUrl: "https://gateway.example.invalid/v1",
			...
			const registry = ModelRegistry.create(authStorage, modelsJsonPath);
			const model = registry.find("custom-gateway", "gpt-5.5");
			expect(model).toBeDefined();
			expect(registry.hasConfiguredAuth(model!)).toBe(true);
			expect(registry.getProviderAuthStatus("custom-gateway")).toEqual({ configured: true, source: "stored" });
			expect(await registry.getApiKeyForProvider("custom-gateway")).toBe("stored-gateway-key");
		});
		...
		test("model schema accepts thinkingLevelMap and split openai-completions cache compat flags", () => {
			writeRawModelsJson({
				demo: {
					baseUrl: "https://example.com/v1",
					apiKey: "DEMO_KEY",
					...
					models: [
						{
							...
							compat: {
								...
								supportsOpenAIPromptCacheRetention: false,
			...
			expect(compat?.supportsOpenAIPromptCacheRetention).toBe(false);
			expect(compat?.supportsAnthropicCacheControlTtl).toBe(true);
		});
	...
	describe("dynamic provider lifecycle", () => {
		test("getProviderDisplayName resolves models.json, registered, OAuth, built-in, and fallback names", () => {
			writeRawModelsJson({
				"local-gateway": {
					name: "Local Gateway",
			...
			expect(registry.getProviderDisplayName("local-gateway")).toBe("Local Gateway");
	...
	describe("API key resolution", () => {
		...
		describe("request-time resolution", () => {
			...
			test("provider auth status reports auth-storage environment variables as configured", () => {
				const originalEnv = process.env.OPENAI_API_KEY;

				try {
					process.env.OPENAI_API_KEY = "status-secret-env-key";

					const registry = ModelRegistry.create(authStorage, modelsJsonPath);
					const status = registry.getProviderAuthStatus("openai");
					...
					expect(JSON.stringify(status)).not.toContain("status-secret-env-key");
				} finally {
					if (originalEnv === undefined) {
			...
			test("provider auth status reports absent apiKey environment variables from models.json as not configured", async () => {
				const envVarName = "TEST_API_KEY_STATUS_ABSENT_98765";
				const originalEnv = process.env[envVarName];
				...
				try {
					...
					expect(await registry.getApiKeyForProvider("custom-provider")).toBeUndefined();
				} finally {
					if (originalEnv === undefined) {
						delete process.env[envVarName];
					} else {
