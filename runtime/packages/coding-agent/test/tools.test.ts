...
describe("Coding Agent Tools", () => {
	...
	describe("edit tool", () => {
		...
		it("should include EACCES for read-only files", async () => {
			...
			// Windows reports EPERM (not EACCES) for read-only files.
		...
		// chmod 0o222 cannot make a file unreadable on Windows (no write-only attribute).
	...
	describe("grep tool", () => {
		...
		it("should treat flag-like patterns as search text", async () => {
			...
			// Patterns are regexes; use forward slashes so Windows path backslashes
			// are not parsed as regex escapes.
	...
	describe("find tool", () => {
		...
		it("should surface directory enumeration permission errors instead of no matches", async () => {
			const def = createFindToolDefinition(testDir, {
				operations: {
					exists: () => true,
					glob: () => {
						const error = new Error("EPERM: operation not permitted, scandir '/protected/Documents'") as NodeJS.ErrnoException;
						error.code = "EPERM";
						throw error;
					},
				},
	...
	describe("ls tool", () => {
		...
		it("should surface directory enumeration permission errors with macOS privacy context", async () => {
			const def = createLsToolDefinition(testDir, {
				operations: {
					exists: () => true,
