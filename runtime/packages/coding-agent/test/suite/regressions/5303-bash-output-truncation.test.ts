...
describe.skipIf(process.platform === "win32")("issue #5303 bash output truncation past exit", () => {
	...
	it("releases within the drain bound when a detached child never stops writing", async () => {
		// A backgrounded writer with gaps shorter than the grace would re-arm it
		// forever; the total post-exit drain is bounded so the tool still returns.
