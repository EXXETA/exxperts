import type { AssistantMessage } from "@exxeta/exxperts-ai";
import { afterEach, describe, expect, it, vi } from "vitest";
import { createHarness, type Harness } from "../harness.js";

type SessionWithCompactionInternals = {
	_checkCompaction: (assistantMessage: AssistantMessage) => Promise<void>;
...
describe("issue #8328 zero-usage auto-compaction", () => {
	...
	it("uses the message estimate when no assistant has reported usage", async () => {
		...
		const runAutoCompactionSpy = vi.spyOn(sessionInternals, "_runAutoCompaction").mockResolvedValue(undefined);
