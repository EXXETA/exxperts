import { Box, type Component, Container, getCapabilities, Image, Spacer, Text, type TUI } from "@exxeta/exxperts-tui";
...
export class ToolExecutionComponent extends Container {
	...
	private sanitizedArgsForFallback(): any {
		...
		const content = typeof this.args.content === "string"
			...
			: "[omitted]";
		return {
			path: String(this.args.path ?? ""),
			content,
			overwrite: this.args.overwrite === true,
		};
	}

	private formatToolExecution(): string {
		let text = theme.fg("toolTitle", theme.bold(this.toolName));
		const content = JSON.stringify(this.sanitizedArgsForFallback(), null, 2);
		if (content) {
			text += `\n\n${content}`;
		}
		const output = this.getTextOutput();
