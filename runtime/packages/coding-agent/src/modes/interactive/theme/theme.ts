...
import type { EditorTheme, MarkdownTheme, SelectListTheme } from "@exxeta/exxperts-tui";
...
export class Theme {
	...
	getThinkingBorderColor(level: "off" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max"): (str: string) => string {
		...
		switch (level) {
			...
			// The tiers above high share one band: they are both "beyond high",
			// and no theme carries a separate colour for the second one.
			case "xhigh":
			case "max":
...
const THEME_KEY = Symbol.for("@exxeta/exxperts-runtime:theme");
...
export function getSettingsListTheme(): import("@exxeta/exxperts-tui").SettingsListTheme {
	return {
