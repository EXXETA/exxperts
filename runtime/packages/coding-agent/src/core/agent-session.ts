...
import type {
	...
	LoopStrategy,
	ThinkingLevel,
} from "@exxeta/exxperts-core";
import { createStrategyStopHook } from "@exxeta/exxperts-core";
import type { AssistantMessage, ImageContent, Message, Model, TextContent } from "@exxeta/exxperts-ai";
import {
	clampThinkingLevel,
...
export interface AgentSessionConfig {
	...
	/** Optional loop strategy for controlling agent iteration limits. */
	loopStrategy?: LoopStrategy;
	/** Optional per-agent context policy (compaction thresholds, preserve instructions, injection rules). */
...
export class AgentSession {
	...
	constructor(config: AgentSessionConfig) {
		...
		if (config.loopStrategy) {
			this._installLoopStrategy(config.loopStrategy);
		}
	...
	/**
	 * Install a loop strategy on the Agent instance.
	 * Sets up shouldStopAfterTurn to delegate to the strategy.
	 * The strategy hook resets its counters on each agent_start event.
	 */
	private _installLoopStrategy(strategy: LoopStrategy): void {
		const hook = createStrategyStopHook(strategy);
		if (!hook) return; // "default" strategy — no hook needed

		this.agent.shouldStopAfterTurn = (ctx) => hook.shouldStopAfterTurn(ctx);

		// Reset iteration counters on each new agent run
		const originalSubscribe = this.agent.subscribe((event) => {
			if (event.type === "agent_start") {
				hook.reset();
			}
		});
		// Store unsubscribe for cleanup if needed (currently lives for session lifetime)
	...
	/** Per-agent context policy (compaction thresholds, preserve instructions, etc.) */
	get contextPolicy(): ContextPolicy | undefined {
	...
	private _rebuildSystemPrompt(toolNames: string[]): string {
		...
		if (this._rawSystemPrompt !== undefined) {
	...
	async compact(customInstructions?: string): Promise<CompactionResult> {
		...
		// Merge caller instructions with per-agent preserve instructions
		const agentPreserve = this._contextPolicy?.compaction?.preserve;
		const effectiveInstructions = customInstructions && agentPreserve
			? `${customInstructions}\n\nAgent-specific preservation: ${agentPreserve}`
			: customInstructions ?? agentPreserve;
		...
		try {
			...
			if (extensionCompaction) {
				...
				// Generate compaction result (includes per-agent preserve instructions)
	...
	private async _runAutoCompaction(reason: "overflow" | "threshold", willRetry: boolean): Promise<void> {
		...
		try {
			...
			// Resolve per-agent compaction preserve instructions
			const agentPreserve = this._contextPolicy?.compaction?.preserve;
			...
			if (extensionCompaction) {
				...
				// Generate compaction result (pass per-agent preserve instructions if set)
	...
	private _bindExtensionCore(runner: ExtensionRunner): void {
		...
		runner.bindCore(
			...
			},
			{
				...
				getContextPolicy: () => this._contextPolicy,
			},
			{
