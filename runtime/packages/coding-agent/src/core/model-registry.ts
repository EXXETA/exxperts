...
import { registerOAuthProvider, resetOAuthProviders } from "@exxeta/exxperts-ai/oauth";
...
const OpenAICompletionsCompatSchema = Type.Object({
	...
	supportsOpenAIPromptCacheRetention: Type.Optional(Type.Boolean()),
	supportsAnthropicCacheControlTtl: Type.Optional(Type.Boolean()),
...
function isLikelyEnvironmentVariableName(value: string): boolean {
	return /^[A-Z_][A-Z0-9_]*$/.test(value) && value.includes("_");
}

function getProviderApiKeyConfigStatus(apiKey: string | undefined): AuthStatus | undefined {
	if (!apiKey) return undefined;
	if (apiKey.startsWith("!")) {
		return { configured: true, source: "models_json_command" };
	}
	if (process.env[apiKey]) {
		return { configured: true, source: "environment", label: apiKey };
	}
	if (isLikelyEnvironmentVariableName(apiKey)) {
		return { configured: false, source: "environment", label: apiKey };
	}
	return { configured: true, source: "models_json_key" };
}

function isProviderApiKeyConfigReady(apiKey: string | undefined): boolean {
	return getProviderApiKeyConfigStatus(apiKey)?.configured === true;
}

function resolveProviderApiKeyConfig(apiKey: string): string | undefined {
	if (!apiKey.startsWith("!") && !process.env[apiKey] && isLikelyEnvironmentVariableName(apiKey)) {
		return undefined;
	}
	return resolveConfigValueUncached(apiKey);
}

function resolveProviderApiKeyConfigOrThrow(apiKey: string, description: string): string | undefined {
	if (!apiKey.startsWith("!") && !process.env[apiKey] && isLikelyEnvironmentVariableName(apiKey)) {
		return undefined;
	}
	return resolveConfigValueOrThrow(apiKey, description);
}
...
export class ModelRegistry {
	...
	private modelsJsonProviderDisplayNames: Map<string, string> = new Map();
	...
	refresh(): void {
		...
		this.modelsJsonProviderDisplayNames.clear();
		...
		try {
			...
			for (const [providerName, providerConfig] of Object.entries(config.providers)) {
				if (providerConfig.name) {
					this.modelsJsonProviderDisplayNames.set(providerName, providerConfig.name);
				}
	...
	private validateConfig(config: ModelsConfig): void {
		...
		for (const [providerName, providerConfig] of Object.entries(config.providers)) {
			...
			if (models.length === 0) {
				...
				// Non-built-in providers with custom models require an endpoint. Auth can
				// come from auth.json, environment, OAuth, or models.json request config.
	...
	hasConfiguredAuth(model: Model<Api>): boolean {
		return (
			...
			isProviderApiKeyConfigReady(this.providerRequestConfigs.get(model.provider)?.apiKey)
		);
	}
		...
		try {
			...
			const apiKey =
				...
				(providerConfig?.apiKey
					? resolveProviderApiKeyConfigOrThrow(providerConfig.apiKey, `API key for provider "${model.provider}"`)
					: undefined);
	...
	getProviderAuthStatus(provider: string): AuthStatus {
		...
		const providerApiKeyStatus = getProviderApiKeyConfigStatus(this.providerRequestConfigs.get(provider)?.apiKey);
		return providerApiKeyStatus ?? authStatus;
	}
	...
	getProviderDisplayName(provider: string): string {
		...
		return (
			...
			oauthProvider?.name ??
			this.modelsJsonProviderDisplayNames.get(provider) ??
	...
	async getApiKeyForProvider(provider: string): Promise<string | undefined> {
		...
		return providerApiKey ? resolveProviderApiKeyConfig(providerApiKey) : undefined;
	}
