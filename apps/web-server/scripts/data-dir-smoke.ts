...
const dataDir = path.join(tempRoot, "nested", "exxperts-data");
{
	...
	assert(env.HOME === dataDir && env.USERPROFILE === dataDir, "HOME/USERPROFILE must point at the data dir");
...
// 6. End to end: a server run against the data dir. The decoy home is what
// HOME pointed at before adoption — it must stay byte-untouched.
...
stateProfiles.adoptStateHome(decoyHome, serverEnv);
serverEnv.EXXPERTS_REAL_HOME = serverEnv.HOME;
