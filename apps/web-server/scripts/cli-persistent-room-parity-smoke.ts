...
try {
	...
	assert(localFilesBoot.allowedToolNames.join(",") === "web_search,fetch_url,read_file,search_file,memory_recall,mcp,read,ls,find,grep,write,edit", "CLI bootstrap local-files allowed tools should include native files and no bash");
	...
	assert(localFilesBashBoot.allowedToolNames.join(",") === "web_search,fetch_url,read_file,search_file,memory_recall,mcp,read,ls,find,grep,write,edit,bash", "CLI bootstrap should include bash for explicit manual Local files bash policy");
