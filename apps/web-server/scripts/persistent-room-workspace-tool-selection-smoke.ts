...
function assertBlockedDisjoint(toolPolicy: { allowedToolNames: string[]; blockedToolNames: string[] }, label: string): void {
	const allowed = new Set(toolPolicy.allowedToolNames);
	assert(toolPolicy.blockedToolNames.every((toolName) => !allowed.has(toolName)), `${label}: blockedToolNames must be disjoint from allowedToolNames`);
}
...
try {
	...
	assert(toolNames(standard) === "ls,find,grep,read,write,edit", "standard selection should enable all current bounded workspace tools");
	assert(registeredToolNames(standard) === "edit,find,grep,ls,read,write", "standard selection should register all bounded workspace tools");
	...
	assert(toolNames(legacyOldStandard) === "ls,find,grep,read,write,edit", "legacy old-standard policy should upgrade to the current all-on bundle");
	const legacySpreadsheetStandard = { ...standard, toolSelection: undefined, allowedToolNames: ["ls", "find", "read", "write_markdown_file", "read_spreadsheet"] };
	delete legacySpreadsheetStandard.toolSelection;
	assert(toolNames(legacySpreadsheetStandard) === "ls,find,grep,read,write,edit", "legacy spreadsheet-standard policy should upgrade to the current all-on bundle");
	const legacyReadonly = { ...standard, toolSelection: undefined, allowedToolNames: ["ls", "find", "read"] };
	delete legacyReadonly.toolSelection;
	assert(toolNames(legacyReadonly) === "ls,find,grep,read,write,edit", "legacy read-only policy should upgrade to the current all-on bundle");
	...
	assert(toolNames(customNoSpreadsheet) === "ls,find,read,write,edit", "stored custom selection naming write_markdown_file should resolve to write and edit");
	assert(registeredToolNames(customNoSpreadsheet) === "edit,find,ls,read,write", "migrated custom selection should register the translated writer tools and no grep");
	...
	assert(toolNames(customNoMarkdown) === "ls,find,read", "stored custom selection naming read_spreadsheet should drop it silently");
	assert(noMarkdownView.writeEnabled === false && customNoMarkdown.modes.write === false, "custom without a writer tool should report write disabled");
	assert(registeredToolNames(customNoMarkdown) === "find,ls,read", "custom read-only selection should register no writer tools");

	const customEditOnly = createPersistentRoomCapabilityPolicy({
		...baseInput,
		conversationId: "custom_edit_only",
		toolSelection: { kind: "custom", allowedToolNames: ["read", "edit"] },
	});
	const editOnlyView = persistentRoomCapabilityPolicyView(customEditOnly);
	assert(editOnlyView.writeEnabled === true && customEditOnly.modes.write === true, "edit alone can change files, so it derives write capability");
	...
	for (const invalidTool of ["bash", "unknown_tool"]) {
	...
	assert(validLocalFilesSelection.kind === "custom" && validLocalFilesSelection.allowedToolNames.join(",") === "read,grep,edit", "Local files custom selection should accept non-bash native tools and drop retired read_spreadsheet");
	for (const invalidLocalFilesTool of ["bash", "unknown_tool"]) {
	...
	assert(liveDefault.allowedToolNames.join(",") === "ls", "live default effective tools should preserve custom subset minus retired names");
	...
	assert(sidecar.toolSelection?.allowedToolNames?.join(",") === "ls", "thread sidecar should store the migrated custom selected tools");
	...
	assert(persistentRoomRuntimeCwdForEffectiveWorkspacePolicy(effective, repoRoot) === fs.realpathSync.native(workspaceB), "bounded room session cwd should be the workspace root realpath, not the repo root");
	assert(persistentRoomRuntimeCwdForEffectiveWorkspacePolicy(null, repoRoot) === repoRoot, "room without a workspace grant keeps the fallback cwd");

	assert(toolPolicy.allowedToolNames.includes("web_search"), "persistent-room tool policy should retain web_search");
	assert(toolPolicy.allowedToolNames.includes("ls"), "persistent-room tool policy should include selected custom workspace tools");
	assert(!toolPolicy.allowedToolNames.includes("find") && !toolPolicy.allowedToolNames.includes("read") && !toolPolicy.allowedToolNames.includes("write"), "persistent-room tool policy should not broaden to unselected workspace tools");
	...
	assert(toolNames(localFilesPolicy) === "read,ls,find,grep,write,edit", "Local files standard policy should expose all non-bash local-files tools");
