...
//   sources are deleted only after every copy landed, so an interruption at
//   any point leaves at least one complete copy and a re-run finishes the
//   job (leftover destinations are rebuilt from the still-complete source);
// - planHomeMove refuses the current home and folders inside the family, and
//   flips to mode "adopt" when the target already holds exxperts data;
...
// 1. migrateStateFamily moves the family and only the family.
{
	...
	// A re-run after an interruption: a leftover destination tree and staging
	// debris are replaced from the still-complete source.
	fs.mkdirSync(path.join(from, ".exxperts"), { recursive: true });
	fs.writeFileSync(path.join(from, ".exxperts", "mine.txt"), "current");
	fs.mkdirSync(path.join(to, ".exxperts.copying-999-stale"), { recursive: true });
	stateProfiles.migrateStateFamily(from, to);
	assert(fs.readFileSync(path.join(to, ".exxperts", "mine.txt"), "utf8") === "current", "a leftover destination must be rebuilt from the source");
	assert(!fs.existsSync(path.join(to, ".exxperts", "app")), "the rebuilt destination must not keep stale content");
...
// 2. planHomeMove: refusals and mode detection.
{
	...
	const fresh = stateProfiles.planHomeMove(home, path.join(unit, "plan-fresh"));
